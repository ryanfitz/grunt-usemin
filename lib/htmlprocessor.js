...
//
var HTMLProcessor = module.exports = function (src, dest, content, revvedfinder, options, logcb) {
...
HTMLProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines) {
    ...
    var addBaseUrl = function(to) {
      if(self.options.baseUrl) {
        return url.resolve(self.options.baseUrl, to);
      }
    ...
    regexps.forEach(function (rxl) {
      ...
      content = content.replace(rxl[0], function (match, src) {
        ...
        var res = match.replace(src, addBaseUrl(filterOut(file)));
